...
const ShowProfileUser = ({ match, history }) => {
    ...
    const fetchCurrentUser = async id => {
        try{
            ...
            const { firstName, lastName, avatar, introduction, description, createdAt } = data;
            setUser({ firstName, lastName, avatar, introduction, description, createdAt });
        }catch(error){
            if(error.response.status === 403){
                ...
                toast.error("Une erreur est survenue lors du chargement des données de l'utilisateur !");
                return history.push("/");
            }
        }
    };
                ...
                <div className="container">
...

                    {
                        bookingsByUser.length > 0 ?
                            <div className="row">
                                { bookingsByUser.map(bookingByUser => <BookingCard booking={ bookingByUser } key={ bookingByUser.id } />) }
                            </div>
                            :
